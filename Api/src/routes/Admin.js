...
app.post("/payments", (req, res) => {
  (async function () {
    try {
      const ClientInvoice = await Invoice.findOne({
        where: { Number: req.body.invoiceNumber },
      });

      if (
        ClientInvoice.Total <= ClientInvoice.Balance ||
        ClientInvoice.Status === "Paid"
      ) {
        res.status(201).send({ message: "Factura Pagada" });
      }

      if (
        ClientInvoice.Total <
        parseInt(ClientInvoice.Balance) + parseInt(req.body.Amount)
      ) {
        res.status(201).send({
          message: `El pago excede el total, se requiere ${
            ClientInvoice.Total - ClientInvoice.Balance
          }`,
        });
      } else {
...
app.get("/invoices:Number", (req, res) => {
  (async function () {
    try {
      const InvoiceForNumber = await Invoice.findOne({
