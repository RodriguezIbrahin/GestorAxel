...
app.post("/singin", (req, res) => {
  (async function () {
    try {
      const ClientSingIn = await Client.findOne({
        where: { DNI: req.body.DNI },
      });

      if (!ClientSingIn) {
        res.status(404).send({ menssage: "Usuario no existente" });
      } else
        bcrypt.compare(
          req.body.Password,
          ClientSingIn.Password,
          function (err, result) {
            if (!err && result) {
              JWT.sign(
                { IsAdmin: ClientSingIn.IsAdmin, DNI: ClientSingIn.DNI },
                AUTH_SECRET,
                function (error, token) {
                  if (error) {
                    ...
                    res.status(200).send({
                      IsAdmin: ClientSingIn.IsAdmi,
                      token,
                      DNI: ClientSingIn.DNI,
                    });
                  }
                }
              );
